...
const BatchRange = {
  ofSize: (min: number, size: number): BatchRange => ({min, max: min + size - 1}),
  ofSizeFromMax: (max: number, size: number): BatchRange => ({min: max - size + 1, max}),
  ...
  private fetchMissingTransactions(
    ...
    return rx
      ...
      .pipe(
        ...
        rxop.concatMap((batchToPick) => {
          console.log(account, 'making a request for transactions', batchToPick)

          return this.fetchTransactions(account, batchToPick.min, batchToPick.max).then(
        ...
        rxop.scan((prev, last) => TransactionHistory.merge(prev, last)),
